...
async function simulateSetK9InstructorRole(
  ...
  const profileRoleKeys = options.profileData?.role_keys
    ? (options.profileData.role_keys as string[])
    : currentProfileId
      ? [currentProfileId]
      : [];
...
test("Phase D.17 (Cenario 14): Falha na compensacao retorna COMPENSATION_FAILED", async () => {
  await assert.rejects(
    () =>
      simulateSetK9InstructorRole("9001", true, {
        authUser: { uid: "uid-test", customClaims: { role: "gestor" } },
