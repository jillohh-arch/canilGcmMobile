...
class _TreatmentExecutionScreenState extends State<TreatmentExecutionScreen> {
  ...
  Future<void> _handlePauseResume(TreatmentProtocol protocol) async {
    final isPaused = protocol.status == TreatmentStatus.paused;

    if (isPaused) {
      final confirmed = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: const Text('Retomar Tratamento'),
          content: Text(
            'Deseja reativar o protocolo de ${protocol.medicationName}?',
          ),
          actions: [
            TextButton(
      ...
      setState(() => _isSubmitting = true);
      try {
        final res = await _gateway.resumeProtocol(
          ResumeTreatmentProtocolCommand(
            dogId: _dogId,
            protocolId: protocol.id,
            operationId: _uuid.v4(),
          ),
        ...
        if (!mounted) return;
        if (res is TreatmentProtocolSuccess) {
          AppFeedback.success(context, 'Tratamento retomado com sucesso!');
        } else if (res is TreatmentProtocolFailure) {
      ...
      final confirmed = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: const Text('Pausar Tratamento'),
          content: Form(
            key: formKey,
            child: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                Text(
                  'Informe o motivo para pausar ${protocol.medicationName}:',
      ...
      setState(() => _isSubmitting = true);
      try {
        final res = await _gateway.pauseProtocol(
          PauseTreatmentProtocolCommand(
            dogId: _dogId,
            protocolId: protocol.id,
            pauseReason: reasonController.text.trim(),
            operationId: _uuid.v4(),
          ),
        );
        if (!mounted) return;
        if (res is TreatmentProtocolSuccess) {
          AppFeedback.success(context, 'Tratamento pausado.');
        } else if (res is TreatmentProtocolFailure) {
          AppFeedback.error(context, res.message);
        }
      } finally {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (protocol.status == TreatmentStatus.active ||
                protocol.status == TreatmentStatus.paused) ...[
              ...
              Align(
                ...
                child: TextButton.icon(
                  ...
                  onPressed:
                      _isSubmitting ? null : () => _handlePauseResume(protocol),
                  icon: Icon(
                    protocol.status == TreatmentStatus.paused
