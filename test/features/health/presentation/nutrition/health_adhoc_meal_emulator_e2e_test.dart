...
void main() {
  ...
  group('Gate 5C.3B Flutter UI Integration E2E (HealthAdhocMealFormSheet -> Gateway -> Emulators)', () {
    if (!enabled) {
      test(
        'skipped fora do orquestrador Emulator (defina HEALTH_NUTRITION_EMULATOR_INTEGRATION=1)',
        () {},
        skip: 'Integração Emulator: rode via flutter test com HEALTH_NUTRITION_EMULATOR_INTEGRATION=1',
      );
      return;
    }
      ...
      (tester) async {
        ...
        controller = HealthNutritionMutationController(
          ...
          onRefreshAfterSuccess: () async {
            refreshCount++;
            print('[Gate5C3BFlutterE2E] Read-after-write refresh triggered count=$refreshCount');
          },
        );
        ...
        print('[Gate5C3BFlutterE2E] UI recebeu resposta de sucesso. mealId=$mealId');
        ...
        print('[Gate5C3BFlutterE2E] CADEIA INTEGRADA COMPROVADA COM SUCESSO.');
        controller.dispose();
      },
    );
  });
...
class _NutritionEmulatorHarness {
  ...
  factory _NutritionEmulatorHarness.fromEnvironment() {
    return _NutritionEmulatorHarness(
      authHost: Platform.environment['FIREBASE_AUTH_EMULATOR_HOST'] ?? '127.0.0.1:9099',
      fsHost: Platform.environment['FIRESTORE_EMULATOR_HOST'] ?? '127.0.0.1:8080',
      fnHost: '127.0.0.1',
      fnPort: 5001,
  ...
  Future<void> signInOperator() async {
    ...
    if (res.statusCode != 200) {
      throw StateError('Auth Emulator login falhou: ${res.statusCode} ${res.body}');
    }
  ...
  static Map<String, dynamic> _decodeFirestoreFields(Map<String, dynamic> fields) {
