...
A **formação** (Fases 1b a 4c — as fases de caixas) é o que esta Parte 4 especifica em detalhe.
...
| **4c** | 4 (quadrado) | variável | não | Busca circular (horário + anti-horário) — **última fase da formação** | 3 horário + 3 anti-horário |
...
- **4c — Busca circular.** Quatro caixas em **quadrado**, instrutor no centro. Ensina a varredura em sentido **horário** (3 consecutivas) e depois **anti-horário** (3 consecutivas). **É a última fase da formação.**

**Concluída a 4c, o cão está formado** (linha → `operational`). O trabalho seguinte, em ambientes reais (veículos, salas, bagagens, áreas abertas), **é manutenção** (3.11). Não existe "fase final" de formação — ela e a manutenção são o mesmo trabalho.
...
- **in_formation** — subindo as fases de caixas (1b … 4c). `current_phase` indica onde está.
- **operational** — **concluiu a 4c** (última fase de caixas) → cão formado. A partir daqui o cão entra no fluxo de **Manutenção (3.11)**; não há mais fases de formação.
...
**Avanço de fase.** Sempre da fase atual para a **próxima na ordem** (1b→2b→2v→3b→3v→4b→4v→4c). Concluir a **4c** (ambas as etapas, horário e anti-horário) **conclui a formação**: a linha vira `operational` e a **manutenção é habilitada** para aquela linha.
...
| `current_phase` | string | código da fase atual: `1b`,`2b`,`2v`,`3b`,`3v`,`4b`,`4v`,`4c`. Concluída a 4c, `status` vira `operational` (não há fase após a 4c) |
...
- Concluir as duas etapas fecha a 4c e **conclui a formação** (cão formado → linha `operational` → manutenção habilitada).
...
## 4.11 — Fim da formação → manutenção

**Não existe "fase final" de formação.** A formação termina na **4c**. Concluída a 4c:
- a linha vai para `status = operational` (cão **formado** naquela linha);
- a **manutenção** (3.11) passa a ser o que o app abre para aquela linha;
- o trabalho em **ambientes reais** (veículos, salas, bagagens, áreas abertas) acontece dentro da **manutenção**, não como fase de formação.

**Onde a manutenção mora (navegação):** o condutor não escolhe "formação ou manutenção". Ele entra em **Treino → Detecção → escolhe a linha** (Drogas / Armas / Cadáver); o **estado da linha decide**: `in_formation` abre o fluxo de formação (4.10); `operational` abre a manutenção (3.11). Linhas distintas podem estar em estágios distintos no mesmo cão.
...
5. **A formação termina na 4c.** Não há "fase final": concluída a 4c, o cão é formado (`operational`) e o trabalho em ambientes reais passa a ser **manutenção** (3.11). A navegação não pede escolha entre formação/manutenção — o estado da linha decide (ver 4.11).
